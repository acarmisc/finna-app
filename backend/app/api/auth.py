...
def require_auth(user: dict[str, Any] = Depends(get_current_user)) -> dict[str, Any]:  # noqa: B008
    """Require authentication dependency."""
    return user


async def require_admin(user: dict[str, Any] = Depends(get_current_user)) -> dict[str, Any]:  # noqa: B008
...
async def exchange_github_code(code: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_504_GATEWAY_TIMEOUT, detail="github_timeout") from e
